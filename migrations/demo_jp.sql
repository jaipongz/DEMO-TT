DROP TABLE IF EXISTS `demo_jp`;
CREATE TABLE `demo_jp` (
  `demo_jp_id` BIGINT,
  ...
  PRIMARY KEY (demo_jp_id, obj_lang),
  INDEX idx_demo_jp_lang (obj_lang),
  INDEX idx_demo_jp_content (obj_content_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

DROP TABLE IF EXISTS `demo_jp_draft`;
CREATE TABLE `demo_jp_draft` (
  `demo_jp_id` BIGINT NOT NULL,
  `title` VARCHAR(255) NOT NULL,
  `email` VARCHAR(255) NULL,
  `age` INT NULL,
  `brand_color` VARCHAR(255) NULL,
  ...
  PRIMARY KEY (demo_jp_id, obj_lang, obj_rev),
  INDEX idx_demo_jp_draft_status (obj_status),
  INDEX idx_demo_jp_draft_state (obj_state),
  INDEX idx_demo_jp_draft_lang (obj_lang),
  INDEX idx_demo_jp_draft_content (obj_content_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Module permissions: demo_jp
DELETE rp
FROM `wcm_role_permissions` rp
INNER JOIN `wcm_permissions` p ON p.id = rp.permission_id
WHERE p.`module` = 'demo_jp';

DELETE FROM `wcm_permissions` WHERE `module` = 'demo_jp';
INSERT IGNORE INTO `wcm_permissions` (`name`, `description`, `module`, `module_name`, `action`) VALUES
('demo_jp.create', 'Can create demo_jp', 'demo_jp', 'Demo JP', 'create'),
('demo_jp.read', 'Can read demo_jp', 'demo_jp', 'Demo JP', 'read'),
('demo_jp.update', 'Can update demo_jp', 'demo_jp', 'Demo JP', 'update'),
('demo_jp.delete', 'Can delete demo_jp', 'demo_jp', 'Demo JP', 'delete'),
('demo_jp.export', 'Can export demo_jp', 'demo_jp', 'Demo JP', 'export'),
('demo_jp.publish', 'Can publish demo_jp', 'demo_jp', 'Demo JP', 'publish');

INSERT IGNORE INTO `wcm_role_permissions` (`role_id`, `permission_id`)
SELECT 1, p.id
FROM `wcm_permissions` p
WHERE p.name IN ('demo_jp.create', 'demo_jp.read', 'demo_jp.update', 'demo_jp.delete', 'demo_jp.export', 'demo_jp.publish');